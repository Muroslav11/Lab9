...
double GetAverageGrade(const Student& s) {
    int thirdMark;
    if (s.specialty == COMPUTER_SCIENCE) thirdMark = s.programming;
    else if (s.specialty == INFORMATICS) thirdMark = s.numericalMethods;
    else thirdMark = s.pedagogy;

    return (s.physics + s.math + thirdMark) / 3.0;
}
...
void Create(Student* p, const int N) {
    for (int i = 0; i < N; i++) {
        ...
        case COMPUTER_SCIENCE:
            p[i].programming = getValidInt(" Програмування (2-5): ", 2, 5);
            break;
        case INFORMATICS:
            p[i].numericalMethods = getValidInt(" Чисельні методи (2-5): ", 2, 5);
            break;
        default:
            p[i].pedagogy = getValidInt(" Педагогіка (2-5): ", 2, 5);
            break;
        }
    }
}
...
void Print(Student* p, const int N) {
    ...
    cout << "| " << setw(2) << "№" << " | " << setw(15) << left << "Прізвище" << " | " << setw(4) << "Курс"
        << " | " << setw(23) << left << "Спеціальність" << " | " << setw(3) << "Фіз" << " | " << setw(3) << "Мат"
        << " | " << setw(4) << "Прог" << " | " << setw(4) << "ЧисМ" << " | " << setw(4) << "Пед" << " | " << setw(7) << "Сер.бал" << " |" << endl;
    cout << string(118, '-') << endl;

    for (int i = 0; i < N; i++) {
        cout << "| " << setw(2) << right << i + 1 << " | " << setw(15) << left << p[i].lastName
            << " | " << setw(4) << right << p[i].course << " | " << setw(23) << left << getSpecName(p[i].specialty)
            << " | " << setw(3) << p[i].physics << " | " << setw(3) << p[i].math << " | ";

        if (p[i].specialty == COMPUTER_SCIENCE)
            cout << setw(4) << p[i].programming << " | " << setw(4) << " " << " | " << setw(4) << " " << " | ";
        else if (p[i].specialty == INFORMATICS)
            cout << setw(4) << " " << " | " << setw(4) << p[i].numericalMethods << " | " << setw(4) << " " << " | ";
        else
            cout << setw(4) << " " << " | " << setw(4) << " " << " | " << setw(4) << p[i].pedagogy << " | ";

        cout << setw(7) << fixed << setprecision(2) << GetAverageGrade(p[i]) << " |" << endl;
    }
    cout << string(118, '=') << endl;
}
...
void PrintIndexSorted(Student* p, int* I, const int N) {
    if (I == nullptr) return;
    cout << "\nІндексна таблиця (Рівень В, Варіант 10):" << endl;
    cout << string(118, '=') << endl;
    cout << "| " << setw(2) << "№" << " | " << setw(15) << left << "Прізвище" << " | " << setw(4) << "Курс"
        << " | " << setw(23) << left << "Спеціальність" << " | " << setw(3) << "Фіз" << " | " << setw(3) << "Мат"
        << " | " << setw(4) << "Прог" << " | " << setw(4) << "ЧисМ" << " | " << setw(4) << "Пед" << " | " << setw(7) << "Сер.бал" << " |" << endl;
    ...
    for (int i = 0; i < N; i++) {
        ...
        cout << "| " << setw(2) << right << i + 1 << " | " << setw(15) << left << p[idx].lastName
            << " | " << setw(4) << right << p[idx].course << " | " << setw(23) << left << getSpecName(p[idx].specialty)
            << " | " << setw(3) << p[idx].physics << " | " << setw(3) << p[idx].math << " | ";

        if (p[idx].specialty == COMPUTER_SCIENCE)
            cout << setw(4) << p[idx].programming << " | " << setw(4) << " " << " | " << setw(4) << " " << " | ";
        else if (p[idx].specialty == INFORMATICS)
            cout << setw(4) << " " << " | " << setw(4) << p[idx].numericalMethods << " | " << setw(4) << " " << " | ";
        else
            cout << setw(4) << " " << " | " << setw(4) << " " << " | " << setw(4) << p[idx].pedagogy << " | ";

        cout << setw(7) << fixed << setprecision(2) << GetAverageGrade(p[idx]) << " |" << endl;
    }
    cout << string(118, '=') << endl;
}
...
int BinSearch(Student* p, const int N, const string name, const int course, const double avg) {
    ...
    while (L <= R) {
        ...
        if (p[m].lastName == name && p[m].course == course && abs(currentAvg - avg) < 1e-9)
            return m;
        if (currentAvg < avg || (abs(currentAvg - avg) < 1e-9 && p[m].course < course) || (abs(currentAvg - avg) < 1e-9 && p[m].course == course && p[m].lastName > name)) {
            L = m + 1;
        }
        else {
            R = m - 1;
...
int main() {
    ...
    do {
        ...
        case 2: Print(p, N); break;
        case 3: Sort(p, N); cout << "Фізичне впорядкування виконано." << endl; break;
        case 4:
        ...
        case 5: {
            ...
            if (res != -1) cout << "Знайдено студента в позиції " << res + 1 << endl;
            else cout << "Шуканого студента не знайдено (виконайте сортування [3] перед пошуком)" << endl;
            break;
        }
        case 0: break;
        default: cout << "Ви ввели помилкове значення!" << endl;
        }
    } while (menuItem != 0);
